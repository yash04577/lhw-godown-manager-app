...
import { getPurchaseByCustomerAsync } from "@/redux/slices/estimatePurchaseSlice";
  
  const purchaseItems = () => {
    const router = useRouter();
    const items = useSelector((state: any) => state?.purchase?.customerOrder);
    ...
    try {
      parsedCustomer = customer ? JSON.parse(customer) : null;
    } catch (error) {
