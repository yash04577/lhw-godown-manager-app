...
import { getPurchaseCustomerAsync } from "@/redux/slices/estimatePurchaseSlice";
import {
...
const TableRow = ({ item, index }: any) => {
  ...
  return (
    <TouchableOpacity onPress={() => handleClick(item)}>
      <View
        className={`bg-white rounded-lg shadow-md p-4 mb-4 w-[90%] mx-auto`}
      >
        <View className="flex-row justify-between items-center border-b border-gray-200 pb-2">
          <Text className="text-gray-700 font-medium">{index + 1}</Text>
          <Text className="text-gray-700">
            Total Orders: {item?.countOfPurchaseOrder}
          </Text>
        </View>
...
const customerPurchase = () => {
  ...
  const estimatePurchase = useSelector((state: any) => state?.purchase?.purchaseCustomer);
...
useEffect(()=>{
  ...
  useEffect(() => {
    dispatch(getPurchaseCustomerAsync(query));
  }, [query]);

  const customers = useSelector(
    (state: any) => state?.estimateSales?.customers
  );
    ...
    <View className="flex-1 h-screen mt-2">
      ...
      <View className="w-full flex justify-center items-center gap-2">
        <View className="w-[90%] flex flex-row">
          <TextInput
            onChangeText={(search) => setQuery((prev)=>({...prev, search:search}))}
